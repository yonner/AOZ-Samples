...
function Errors( aoz )
{
	...
	this.errors=
	{
		...
		[
			...
			"wave_0_and_1_are_reserved:Les son 0 et 1 sont réservées",
...
Errors.prototype.getError = function( error, params )
{
	var self = this;
	if ( typeof error == 'string' )
		error = { error: error, params: params ? params : [] };
	var id = error.error + ':';
		...
		if ( errorList )
		{
			...
			if ( number >= 0 )
			{
				...
				for ( var p = 0; p < error.params.length; p++ )
					message = self.aoz.utilities.replaceStringInText( message, '%' + ( p + 1 ), error.params[ p ] );
